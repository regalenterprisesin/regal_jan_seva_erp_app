...
const InvoicePage: React.FC = () => {
  ...
  // Staging for current item entry
  ...
  const addServiceToList = () => {
    ...
    const s = services.find(service => service.id === currentServiceId);
    if (!s) return;
    ...
    const newItem: JobItem = {
      ...
      status: status // Use the status from the staging area
    };
  ...
  const handleCommitInvoice = async () => {
    ...
    try {
      ...
      const updatedJobs = await db.jobs.all();
      setJobs(updatedJobs.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()));
  ...
  const styles = `
    ...
    @media print {
      ...
      #receipt-container-invoice, #receipt-container-invoice * { visibility: visible; }
      #receipt-container-invoice { position: absolute; left: 0; top: 0; width: 80mm; }
    }
  `;
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[15px] border border-slate-100 dark:border-slate-800 shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 duration-500">
          <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 flex justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tight">{editingId ? 'Edit Invoice' : 'Invoice Terminal'}</h1>
                <p className="text-sm text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest mt-1">Unified Billing Workspace</p>
              </div>
            </div>
          ...
          <div className="p-8 space-y-10">
            ...
            {/* Item Staging & List - Only visible if customer is selected */}
            {isCustomerSelected && (
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-12 gap-4 items-end bg-white dark:bg-slate-950 p-6 rounded-[15px] border border-blue-500/20 shadow-xl">
                    ...
                    <div className="col-span-6 lg:col-span-1 space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block ml-1">Qty</label>
                      <input 
                        type="number" 
                        min="1" 
                        value={quantity} 
                    ...
                    <div className="col-span-6 lg:col-span-2 space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block ml-1">Unit Rate (₹)</label>
                      <input 
                        type="number" 
                        value={rate} 
                    ...
                    <div className="col-span-6 lg:col-span-1 space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block ml-1">Disc. (₹)</label>
                      <input 
                        type="number" 
                        value={discount} 
                    ...
                    <div className="col-span-12 lg:col-span-2 space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block ml-1">Workflow</label>
                      <select 
                        value={status}
                ...
                <div className="space-y-6">
                    ...
                    <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                        {items.length === 0 ? (
                          <tr>
                            <td colSpan={6} className="px-8 py-20 text-center text-sm font-bold text-slate-300 uppercase tracking-widest italic">
                              Bill is empty. Staging items above will populate this ledger.
                            </td>
                          </tr>
                        ) : (
                          items.map((item, idx) => (
                            <tr key={idx} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                              ...
                              <td className="px-6 py-4 text-center text-sm font-bold text-slate-500">₹{item.unitPrice}</td>
                              <td className="px-6 py-4 text-center text-sm font-black text-slate-800 dark:text-white">{item.quantity}</td>
                              <td className="px-6 py-4 text-center text-sm font-black text-rose-500">₹{item.discount}</td>
                              <td className="px-6 py-4 text-right text-sm font-black text-slate-900 dark:text-white tracking-tighter">₹{item.subtotal}</td>
                              <td className="px-8 py-4 text-right">
                                <button onClick={() => removeItem(idx)} className="p-2 text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all">
                                  <Trash2 size={16}/>
                                </button>
                              </td>
                            </tr>
                  ...
                  <div className="flex justify-end pr-8">
                    <div className="w-full max-w-xs space-y-2">
                      <label className="text-sm font-bold text-emerald-600 uppercase tracking-widest block ml-1">Amount Paid (₹)</label>
                      <div className="relative">
                        <IndianRupee className="absolute left-4 top-1/2 -translate-y-1/2 text-emerald-600/50" size={20} />
                        <input 
                          type="number"
                          value={paidAmount}
        ...
        <div className={`fixed inset-0 z-[100] bg-white pointer-events-none flex justify-center ${isGeneratingPDF ? 'opacity-100 overflow-visible' : 'opacity-0 print:opacity-100 print:relative print:z-auto print:block print:w-full'}`}>
          <div id="receipt-container-invoice" className="w-[80mm] p-6 bg-white text-black font-sans border border-gray-100 shadow-sm">
            <div className="text-center mb-6 space-y-1">
              <div className="inline-block bg-black text-white px-2 py-1 rounded font-black text-sm mb-1">RE</div>
              <h2 className="text-lg font-black uppercase">Regal Jan Seva Kendra</h2>
              <p className="text-[9px] font-bold uppercase tracking-widest text-gray-500">Innovation is our Motto</p>
            </div>
            <div className="border-t border-b border-black border-dashed py-3 my-4 space-y-1">
              <div className="flex justify-between text-[8px] font-black uppercase"><span>ID: #{receiptJob?.id.slice(-6).toUpperCase()}</span><span>Date: {new Date(receiptJob?.createdAt || '').toLocaleDateString('en-IN')}</span></div>
            </div>
            <div className="mb-4 space-y-0.5">
              <p className="text-[7px] font-black uppercase text-gray-400">Account Holder:</p>
              <h3 className="text-xs font-black uppercase">{customers.find(c => c.id === receiptJob?.customerId)?.name || 'Walk-in'}</h3>
              <p className="text-[8px] font-mono">{customers.find(c => c.id === receiptJob?.customerId)?.phone}</p>
            </div>
            <table className="w-full text-left mb-4">
              <thead className="border-b border-black text-[7px] font-black uppercase"><tr><th className="py-1">Item</th><th className="py-1 text-center">Qty</th><th className="py-1 text-right">Amt</th></tr></thead>
              <tbody>
                {receiptJob?.items.map((item, idx) => (
                  <tr key={idx} className="text-[8px] border-b border-gray-50">
                    <td className="py-1.5"><div className="font-bold uppercase">{services.find(s => s.id === item.serviceId)?.name || 'Custom'}</div></td>
                    <td className="py-1.5 text-center">{item.quantity}</td>
                    <td className="py-1.5 text-right font-bold">₹{item.subtotal}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t border-black"><td colSpan={2} className="py-2 font-black text-[10px] uppercase">Net Total</td><td className="py-2 text-right font-black text-[10px]">₹{receiptJob?.totalAmount}</td></tr>
                <tr className="text-[8px] font-bold text-gray-600"><td colSpan={2} className="uppercase">Cash Paid</td><td className="text-right">₹{receiptJob?.paidAmount}</td></tr>
                <tr className="text-[8px] font-black text-red-600"><td colSpan={2} className="uppercase">Due Balance</td><td className="text-right">₹{receiptJob?.balance}</td></tr>
              </tfoot>
            </table>
            <div className="text-center mt-6 pt-4 border-t border-gray-100 border-dashed">
               <p className="text-[7px] text-gray-400 italic">Authorized CSC ERP Transaction.</p>
            </div>
          </div>
        </div>
      )}
