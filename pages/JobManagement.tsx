...
const JobManagement: React.FC = () => {
  ...
  const inputClass = "w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs font-bold outline-none focus:border-blue-500 appearance-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none";
  const selectClass = "w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs font-bold outline-none focus:border-blue-500 appearance-none bg-none";
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center print:hidden">
        <div className="flex bg-white dark:bg-slate-900 p-1 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <button onClick={() => setView('LIST')} className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${view === 'LIST' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}>Workflow List</button>
          <button onClick={() => setView('BOARD')} className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${view === 'BOARD' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}>Board View</button>
        </div>
      </div>

      {view === 'LIST' ? (
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl overflow-hidden print:hidden">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-950/50 border-b border-slate-100 dark:border-slate-800">
                <th 
                  className="px-8 py-5 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-colors"
                  onClick={() => toggleSort('customerName')}
                >
                  <div className="flex items-center">Customer <SortIcon field="customerName" /></div>
                </th>
                <th 
                  className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-colors"
                ...
                <tr key={task.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                  <td className="px-8 py-5">
                    <div className="flex items-center space-x-3">
                      <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[10px] font-black text-blue-600 dark:text-blue-400 border border-slate-200 dark:border-slate-700 shrink-0">
                        {task.customerName[0]}
                      </div>
                      <div className="flex flex-col">
                        <span className="text-sm font-black text-slate-900 dark:text-slate-100">{task.customerName}</span>
                        <span className="text-[10px] font-bold text-slate-500">{task.customerPhone}</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <span className="text-[11px] font-mono text-blue-500 font-black tracking-widest uppercase">
                      {task.customerAadhaar ? task.customerAadhaar.replace(/(\d{4})/g, '$1 ').trim() : 'NO AADHAAR'}
                    </span>
                  </td>
                  <td className="px-6 py-5">
                    <div className="font-black text-slate-900 dark:text-slate-100 truncate max-w-xs">{task.serviceName}</div>
                    <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">ID: #{task.jobId.slice(-6).toUpperCase()}</div>
                  </td>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex items-center space-x-2">
                      <select 
                        ...
                        className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-lg border outline-none bg-transparent cursor-pointer transition-all ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 print:hidden">
          {(['PENDING', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED'] as JobStatus[]).map(status => (
            <div key={status} className="bg-white dark:bg-slate-900/50 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 min-h-[500px] shadow-sm">
              <h4 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-6 flex items-center justify-between px-2">
                {status.replace('_', ' ')}
                <span className="bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded-full text-[9px] border border-slate-200 dark:border-slate-700">{flattenedTasks.filter(t => t.status === status).length}</span>
              </h4>
              <div className="space-y-4">
                {flattenedTasks.filter(t => t.status === status).map(task => (
                  <div key={task.id} onClick={() => openModal(task.originalJob)} className="bg-slate-50 dark:bg-slate-900 p-5 rounded-2xl border border-transparent dark:border-slate-800 cursor-pointer hover:border-blue-500/50 hover:bg-white dark:hover:bg-slate-800 transition-all group relative shadow-sm">
                    <p className="text-xs font-black text-slate-900 dark:text-slate-100 mb-1">{task.serviceName}</p>
                    <p className="text-[10px] font-bold text-slate-500 dark:text-slate-500 uppercase tracking-widest mb-1">{task.customerName}</p>
                    <div className="flex justify-between items-center pt-3 border-t border-slate-200 dark:border-slate-800/50">
                       <span className="text-[10px] font-black text-slate-400 dark:text-white">ID: #{task.jobId.slice(-4)}</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-black/80 backdrop-blur-md print:hidden">
          <div className="bg-white dark:bg-slate-950 w-full max-w-[98vw] rounded-[3rem] shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden animate-in zoom-in duration-300">
            <div className="px-10 py-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
              <div>
                ...
                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-[0.2em]">High-Efficiency Multi-Service Entry</p>
              </div>
              <button onClick={closeModal} className="p-2.5 bg-white dark:bg-slate-800 rounded-2xl text-slate-400 hover:text-slate-900 transition-all shadow-sm"><X size={20} /></button>
            </div>
            <div className="p-8 flex flex-col space-y-8 max-h-[75vh] overflow-y-auto custom-scrollbar">
              <div className="bg-slate-50 dark:bg-slate-900/30 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 grid grid-cols-12 gap-8 items-center">
                ...
                <div className="col-span-4 space-y-2">
                  ...
                  <div className="flex items-center space-x-3 bg-white dark:bg-slate-950 px-5 py-3.5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm min-h-[52px]">
                    <Fingerprint size={18} className="text-slate-400" />
                    <span className="text-xs font-mono font-black tracking-widest text-slate-700 dark:text-slate-200">{currentCustomer?.aadhaarNumber ? currentCustomer.aadhaarNumber.replace(/(\d{4})/g, '$1 ').trim() : '0000 0000 0000'}</span>
                  </div>
                </div>
                <div className="col-span-4 space-y-2">
                  ...
                  <div className="flex items-center space-x-3 bg-white dark:bg-slate-950 px-5 py-3.5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm min-h-[52px]">
                    ...
                    <span className="text-xs font-bold text-slate-700 dark:text-slate-200">{currentCustomer?.phone || '+91 00000 00000'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white dark:bg-slate-950 p-6 rounded-[2.5rem] border border-blue-100 dark:border-blue-900/30 shadow-xl grid grid-cols-12 gap-4 items-end">
                  ...
                  <div className="col-span-2"><label className="block text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Net Line Total</label><div className="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 text-xs font-black text-slate-900 dark:text-white border border-slate-200 dark:border-slate-800">₹{currentItem.subtotal}</div></div>
                  ...
                  <div className="col-span-1"><button onClick={handleAddStagedItem} className="w-full p-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all shadow-lg active:scale-95 flex items-center justify-center" title="Push to Bill"><Plus size={22} /></button></div>
                </div>
              </div>

              <div className="flex-1 bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 overflow-hidden shadow-sm min-h-[200px] overflow-y-auto">
                <table className="w-full text-left"><thead className="bg-slate-50 dark:bg-slate-950/50 text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800 sticky top-0 z-10"><tr><th className="px-8 py-4">Bill Item Specification</th><th className="px-6 py-4 text-center">Unit Price</th><th className="px-6 py-4 text-center">Qty</th><th className="px-6 py-4 text-center">Discount</th><th className="px-6 py-4 text-right">Net Value</th><th className="px-8 py-4 text-right">Action</th></tr></thead>
                  <tbody className="divide-y divide-slate-50 dark:divide-slate-800">{(formData.items || []).length === 0 ? (<tr><td colSpan={6} className="px-8 py-12 text-center text-xs font-bold text-slate-300 uppercase tracking-widest italic">No operational data queued</td></tr>) : (formData.items?.map((item, idx) => (<tr key={idx} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors"><td className="px-8 py-4"><div className="text-xs font-black text-slate-800 dark:text-slate-200 uppercase">{getService(item.serviceId)?.name || 'Custom'}</div></td><td className="px-6 py-4 text-center text-xs font-bold text-slate-500">₹{item.unitPrice}</td><td className="px-6 py-4 text-center text-xs font-black text-slate-800 dark:text-white">{item.quantity}</td><td className="px-6 py-4 text-center text-xs font-black text-rose-400">₹{item.discount}</td><td className="px-6 py-4 text-right text-xs font-black text-slate-900 dark:text-white tracking-tighter">₹{item.subtotal}</td><td className="px-8 py-4 text-right"><button onClick={() => removeStagedItem(idx)} className="p-1.5 text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={14}/></button></td></tr>)))}</tbody>
                </table>
              </div>
            </div>
            ...
            <div className="px-10 py-6 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800 flex flex-col md:flex-row justify-between items-center gap-8">
              <div className="flex items-center space-x-12">
                ...
                <div className="flex flex-col"><span className="text-[10px] font-black text-slate-800 dark:text-white uppercase tracking-[0.2em] mb-0.5">Net Amount to be Paid</span><div className="flex items-center text-4xl font-black text-slate-900 dark:text-white tracking-tighter"><IndianRupee size={24} className="mr-1 opacity-20" /> {totals.netTotal}</div></div>
                <div className="flex items-center space-x-8 bg-white dark:bg-slate-950 px-8 py-3 rounded-[1.5rem] border border-slate-200 dark:border-slate-800 shadow-inner">
                   <div className="flex flex-col"><label className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] mb-1">Cash Received (₹)</label><input type="number" value={formData.paidAmount} onChange={(e) => setFormData({...formData, paidAmount: Number(e.target.value)})} className="w-28 bg-transparent text-xl font-black text-emerald-600 outline-none appearance-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" placeholder="0" /></div>
              ...
              <div className="flex space-x-4 w-full md:w-auto"><button onClick={closeModal} className="px-6 py-3.5 font-black text-slate-400 uppercase tracking-widest text-[9px] hover:text-slate-900 dark:hover:text-white transition-colors">Discard</button><button onClick={handleSave} className="bg-slate-900 dark:bg-white text-white dark:text-black px-10 py-3.5 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-blue-600 dark:hover:bg-blue-500 hover:text-white transition-all shadow-xl active:scale-95 flex items-center justify-center space-x-2"><Save size={18} /><span>Sync & Commit</span></button></div>
            </div>
          </div>
        </div>
      )}
      ...
      {(receiptJob || isGeneratingPDF) && (
        <div className={`fixed inset-0 z-[100] bg-white pointer-events-none flex justify-center ${isGeneratingPDF ? 'opacity-100 overflow-visible' : 'opacity-0 print:opacity-100 print:relative print:z-auto print:block print:w-full'}`}>
          <div id="receipt-container" className="w-[80mm] p-6 bg-white text-black font-sans border border-gray-100 shadow-sm">
            ...
            <div className="mb-4 space-y-0.5"><p className="text-[7px] font-black uppercase text-gray-400">Account Holder:</p><h3 className="text-xs font-black uppercase">{getCustomer(receiptJob?.customerId || '')?.name}</h3><p className="text-[8px] font-mono">{getCustomer(receiptJob?.customerId || '')?.phone}</p></div>
            <table className="w-full text-left mb-4"><thead className="border-b border-black text-[7px] font-black uppercase"><tr><th className="py-1">Item</th><th className="py-1 text-center">Qty</th><th className="py-1 text-right">Amt</th></tr></thead>
              <tbody>{receiptJob?.items.map((item, idx) => (<tr key={idx} className="text-[8px] border-b border-gray-50"><td className="py-1.5"><div className="font-bold uppercase">{getService(item.serviceId)?.name}</div></td><td className="py-1.5 text-center">{item.quantity}</td><td className="py-1.5 text-right font-bold">₹{item.subtotal}</td></tr>))}</tbody>
              <tfoot><tr className="border-t border-black"><td colSpan={2} className="py-2 font-black text-[10px] uppercase">Net Total</td><td className="py-2 text-right font-black text-[10px]">₹{receiptJob?.totalAmount}</td></tr><tr className="text-[8px] font-bold text-gray-600"><td colSpan={2} className="uppercase">Cash Paid</td><td className="text-right">₹{receiptJob?.paidAmount}</td></tr><tr className="text-[8px] font-black text-red-600"><td colSpan={2} className="uppercase">Due Balance</td><td className="text-right">₹{receiptJob?.balance}</td></tr></tfoot>
            </table><div className="text-center mt-6 pt-4 border-t border-gray-100 border-dashed"><p className="text-[7px] text-gray-400 italic">Authorized CSC ERP Transaction.</p></div>
          </div>
        </div>
      )}
    </div>
