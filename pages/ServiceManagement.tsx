...
import { Search, Plus, Edit, Trash2, X, Save, Settings, IndianRupee, LayoutGrid, List, Loader2, Filter, Download, Upload } from 'lucide-react';
...
const ServiceManagement: React.FC = () => {
  ...
  const handleExportExcel = () => {
    try {
      ...
      console.error(err);
      alert('Failed to export service data.');
    } finally {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (const item of importedItems) {
          if (item.name) {
            await db.services.save(item);
            count++;
          }
        }

        const freshData = await db.services.all();
        setServices(freshData);
        alert(`Successfully imported ${count} services into the catalog.`);
      } catch (err) {
        console.error(err);
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Erase this service from the catalog?')) {
